  ...
  return (
    <div className="flex flex-col gap-5 lg:flex-row">
      ...
      <div className="flex flex-col items-center gap-3 lg:flex-row">
        <p className="rounded-full bg-neutral-100 px-5 py-2 dark:bg-neutral-900/25">
          {t("professional")}
        </p>
        ...
        {professional.map((item) => (
          <Link
            key={item.name}
            href={item.href}
            className="text-neutral-900 hover:text-red-600 dark:text-white dark:hover:text-red-600"
          >
            {item.name}
          </Link>
        ))}
